...
const getDriveTimeFromGoogle = require('./getDriveTimeFromGoogle.js');
...
const go = async () => {
  ...
  asyncForEach(routeData, async (listRouteData, i) => {
    if (i < 2) {
      ...
      try {
        routeDirectionData = await getDriveTimeFromGoogle({ routeStartPoint: listRouteData.startlatlng });
        console.log('routeDirectionData', routeDirectionData);
      } catch (err) {
        console.log('google err');
        console.log(err);
      }

      let combinedRouteData = {...listRouteData, directions: routeDirectionData};
      try {
        await saveRouteData({newRouteData: combinedRouteData});
      } catch(err) {
        console.log('Save google update err', listRouteData.title);
        console.log(err);
      }
    }
  });
